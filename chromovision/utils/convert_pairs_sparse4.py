...
mat={}  # dictionary object to keep in memory the contacts (erased every chunk)
MAT_CHRO = {}
for c1, c2 in list_combi_chrs :
    MAT_CHRO[c1, c2] = coo_matrix((N, N), dtype=np.int8)
...
with open(filename1) as f: # open the file for reading output alignment file
    ...
    for line in f: # iterate over each line
        ...
        if i % chunksize == 0 :
            ...
            if option_inter == 1 :
                for c1, c2 in list_combi_chrs:
                    # We subset in the dict mat the lines belonging to chrm c:
                    dict_chro = {k: v for k, v in mat.items() if k[0]==c1 and k[2]==c2}
                    list_x = []
                    list_y = []
                    list_values = []
                    for k, v in dict_chro.items() : 
                        list_x.append(k[1])
                        list_y.append(k[3])
...
sparse_matrix = scipy.sparse.load_npz('/home/axel/chr16_chr17_sparse_matrice_10000.txt.npz')
