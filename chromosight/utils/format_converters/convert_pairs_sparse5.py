...
chunksize = (
    1000000
)  # number of lines put into dict, to adapt for RAM consumption
option_inter = (
    1
)  #  option to choose or not to computer inter-chrm sparse matrices
...
list_combi_chrs = list(
    itertools.combinations_with_replacement(list_all_chrms, 2)
)
...
for c1, c2 in list_combi_chrs:
    mat[
        ...
        {}
    )  # temporary dictionary object to keep in memory the contacts (erased every chunk)
...
with open(filename1) as f:  # open the file for reading output alignment file
    ...
    for line in f:  # iterate over each line
        i += 1
        chr1, locus1, sens1, indice1, chr2, locus2, sens2, indice2 = (
            line.split()
        ...
        if (
            chr1 != chr2
            and chr1 in list_all_chrms
            and chr2 in list_all_chrms
            and option_inter == 1
        ):
            if (
                f_num[chr1] > f_num[chr2]
            ):  # we inverse to have always the same order
                chr0 = chr1
                chr1 = chr2
                chr2 = chr0
                locus = locus1
            ...
            if option_inter == 1:
                for c1, c2 in list_combi_chrs:
                    ...
                    mat_chro[c1, c2] = mat_chro[c1, c2] + coo_matrix(
                        (list_v, (list_x, list_y)), shape=(N, N)
                    )
                ...
                for c1, c2 in list_combi_chrs:
                    mat[
                        ...
                        {}
                    )  # dictionary object to keep in memory the contacts (erased every chunk)
                list_x.clear()
                list_y.clear()
                list_v.clear()
            else:  # intra only
...
for c1, c2 in mat_chro.keys():
    ...
    if len(mat_chro[c1, c2].data) > 0:
        ...
        scipy.sparse.save_npz(
            c1 + "_" + c2 + "_sparse_matrice_" + str(bin_size) + ".txt",
            mat_chro[c1, c2],
            compressed=True,
        )

# Test and plot:
sparse_matrix = scipy.sparse.load_npz(
    "/home/axel/Bureau/Hela/sparse_objects/chr1_chr1_sparse_matrice_10000.txt.npz"
)
plt.spy(sparse_matrix, markersize=0.005)


sparse_matrix = scipy.sparse.load_npz(
    "/media/axel/RSG4/diverse_yeast_data/quiescence_2019/sparse_objects_log/chrM_chrM_sparse_matrice_200.txt.npz"
)
plt.figure(1)
...
sparse_matrix = scipy.sparse.load_npz(
    "/media/axel/RSG4/diverse_yeast_data/quiescence_2019/sparse_objects_quiescence/chrM_chrM_sparse_matrice_200.txt.npz"
)
sparse_matrix = sparse_matrix + np.transpose(sparse_matrix)
plt.figure(2)
mn = hcs.normalize_sparse(sparse_matrix)
...
sparse_matrix = scipy.sparse.load_npz(
    "/home/axel/chr1_chr1_sparse_matrice_2000.txt.npz"
)
